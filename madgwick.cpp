...
#define GYRO_MEAN_ERROR PI * (5.0 / 180.0) // 5 deg/s gyroscope measurement error (in rad/s)  *from paper*
...
void MadgwickFilter::compute(double ax, double ay, double az, double gx, double gy, double gz, double dt)
{
  ...
  Quaternion q_est_dot = Quaternion(0.0, 0.0, 0.0, 0.0);            // used as a place holder in equations 42 and 43
  ...
  Quaternion q_a = Quaternion(0, ax, ay, az);    // equation (24) raw acceleration values, needs to be normalized
